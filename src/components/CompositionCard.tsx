...
export default function CompositionCard({
  ...
  return (
    <div
      className={`grid gap-4 px-4 py-4 md:grid-cols-[3rem_5rem_minmax(0,1fr)_13rem] md:items-center md:px-4 ${
        composition.is_featured ? 'border-l-4 border-[#e4ae00] bg-white/[0.06]' : ''
      }`}
    >
      <span className="text-xl font-semibold text-[#e4ae00]">{String(index + 1).padStart(2, '0')}</span>
      ...
      <div className="flex items-center justify-start gap-4 md:justify-end">
        {composition.is_featured && (
          <span className="hidden border border-[#e4ae00]/60 px-2 py-1 text-[9px] font-semibold uppercase tracking-[0.14em] text-[#e4ae00] lg:inline-block">
