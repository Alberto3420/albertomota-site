...
- Edge Functions are **not** deployed by the push; deploy them separately. The installed CLI (1.207.9) requires Docker, so use `npx --yes supabase@latest functions deploy <name> --project-ref vszdpaithvzsuheaftek --use-api` (add `--no-verify-jwt` for `suno-callback` and `asaas-webhook`, which are called by third parties without a Supabase JWT).
...
2. *Paid AI music generation* — the user's dashboard (`pages/Dashboard.tsx`, with `pages/MusicStudio.tsx` embedded as the "Gerar música" tab). Flow: the browser calls Edge Function `generate-music` → it inserts a `music_projects` row (`payment_pending`) plus two `music_versions` rows and creates an Asaas customer + charge (`billingType: UNDEFINED`, so the hosted checkout offers Pix/card), stores it in `music_payments` and returns `paymentUrl` → the user pays → Asaas calls `asaas-webhook` (`verify_jwt = false`, authenticated by the `asaas-access-token` header vs `ASAAS_WEBHOOK_TOKEN`) → on `PAYMENT_CONFIRMED/RECEIVED` it atomically flips the payment `pending→confirmed` (idempotency guard) and calls `startSunoGeneration` (`functions/_shared/suno.ts`) → Suno posts progress to `suno-callback`, which updates `music_versions` (audio URL, clip id, `duration`) and rolls the project status up. **Generation must only ever start from the webhook or the free path** — never from the browser. E-mails listed in the `FREE_EMAILS` secret skip payment and generate directly. `ASAAS_ENV` selects sandbox (default) vs `production`; secrets `ASAAS_API_KEY`, `ASAAS_WEBHOOK_TOKEN`, `FREE_EMAILS`, `SUNO_API_KEY` live in Supabase. The dashboard polls every 15 s while any project is `generating`/`payment_pending`. `suno-credits` returns the remaining Suno balance shown in the dashboard header.
